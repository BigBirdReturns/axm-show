...
function CompileMode({spec,onComplete}){
  ...
  const _compileLive=useCallback(async()=>{
    setState("running");
    addLog("Show Compiler v2.0.0  ·  axm-show-server");
    addLog("Suite: axm-hybrid1 (Ed25519 \u2016 ML-DSA-44, hybrid post-quantum)"); await delay(100);
    ...
    try{
      ...
      addLog("Signing manifest with axm-hybrid1 (Ed25519 + ML-DSA-44/FIPS 204)…");
  ...
  const _compileDemo=useCallback(async()=>{
    ...
    addLog("Suite: axm-hybrid1 (Ed25519 \u2016 ML-DSA-44, hybrid post-quantum)"); await delay(200);
    ...
    addLog("Signing manifest with axm-hybrid1 (Ed25519 + ML-DSA-44/FIPS 204)…  ← simulated");
    ...
    const shardId=`sh1_${merkle.slice(0,64)}`;
...
function InspectMode({shard}){
  ...
  const manifest=shard.manifest||{spec_version:"1.0.0",shard_id:shardId,suite:"axm-hybrid1",publisher:{id:"@axm_show",name:"AXM Show Compiler",created_at:timestamp},metadata:{namespace:"embodied/show",title:`${spec.config.show_name} — ${spec.venue.name}`},integrity:{algorithm:shard._live?"blake3":"sha256-demo",merkle_root:merkleRoot},sources:[{path:"content/source.txt",hash:shard._live?merkleRoot:"(demo — not a real content hash)"}],statistics:{claims:claims.length,entities:subjects.length},extensions:["ext/temporal@1"]};
